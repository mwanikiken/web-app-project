...
if (!isset($_SESSION['Email1']) && !isset($_SESSION['studname'])) {
    header("Location: index.html");
}else{
  $email = $_SESSION['Email1'];
  $query=mysqli_query($conn,"SELECT * FROM `student` WHERE `Email_Address`='$email'")or die(mysqli_error());
  $row=mysqli_fetch_array($query);
  $filter = $_SESSION['studname'];
}
?>
<html>
<head>
    <meta charset="utf-8">
    <title>Student Management - Student Homepage</title>
    ...
    <div class="container-fluid bg-primary px-0 px-md-5 mb-5">
        <div class="row align-items-center px-3">
            <div class="col-lg-6 text-center text-lg-left">
                <h1 class="display-3 font-weight-bold text-white">Welcome Student, <?php echo $row['Fullname']; ?></h1>
                <a href="#about" class="btn btn-secondary mt-1 py-3 px-5">Learn More</a>
            </div>
            <div class="col-lg-6 text-center text-lg-right">
                <img class="img-fluid mt-5" src="img/hero1.jpg" alt="">
            </div>
        </div>
    </div>
    <!-- Header End -->
...
$sql = "SELECT `Student_ID`,`Fullname`, `Email_Address` FROM `student`  WHERE `Student_ID` = '$filter'";
...
$sql = "SELECT `Grade_ID`, `Subject`, `Marks` FROM `grades` WHERE `Student_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
                        ...
                        <div class="card-body rounded-bottom bg-primary p-5">
                            <form action="insertion.inc.php" method="post">
                              <div class="form-group">
                                    ...
                                    <input type="hidden" value="<?php echo $filter; ?>" required="required" name="sid"/>
                              </div>
                                <div class="form-group">
                                ...
                                <div>
                                    <button class="btn btn-secondary btn-block border-0 py-3" type="submit" name="ups">Update</button>
                                </div>
                            </form>
                        </div>
                    </div>
